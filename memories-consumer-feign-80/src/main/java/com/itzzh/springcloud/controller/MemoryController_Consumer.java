...
import com.itzzh.springcloud.service.MemoryClientService;
...
/**
 * 利用feign调用服务提供者CRUD，在此处理业务逻辑。
 */
@Controller
@RequestMapping("/memory")
public class MemoryController_Consumer {

    @Autowired
    private MemoryClientService service;

    @RequestMapping("/memories")
    public String memories(Map<String,Object> map){
        //获取数据，做多不超过50个
        List<Memory> memoryList = service.getAllMemories();
    ...
    public String queryMemoryById(@PathVariable("id") Long memoryId, Map<String,Object> map) {
        Memory memory = service.getMemory(memoryId);
